...
export default function ProfileScreen() {
  ...
  const onLogout = () => {
    Alert.alert(
      'Çıkış yap',
      'Oturumunu kapatmak istediğinden emin misin?',
      [
  ...
  const appVersion =
    Constants.expoConfig?.version ?? Constants.expoConfig?.runtimeVersion ?? '—'
    ...
    <SafeAreaView edges={['bottom']} style={{ flex: 1, backgroundColor: t.colors.surface.canvas }}>
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 48 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ marginTop: 24 }}>
          <Button label="Çıkış Yap" variant="outline" tone="danger" size="lg" onPress={onLogout} fullWidth />
        </View>
      </ScrollView>
    </SafeAreaView>
  )
...
function StatCol({ label, value }: { label: string; value: number }) {
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', gap: 6 }}>
      <Text style={{ fontFamily: 'Fraunces_700Bold', fontSize: 26, lineHeight: 30, color: t.colors.text.primary, fontVariant: ['tabular-nums'] }}>
        {value}
      </Text>
