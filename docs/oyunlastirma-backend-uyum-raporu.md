...
## 9. Faz 3 — event wiring durumu (PR #67, 2026-06-21 güncel)

`gamification/event` mobil tarafta bağlandı (`hooks/use-award.ts` + `lib/gamification/
award-events.ts` saf builder + `lib/api/gamification.ts`). Olaylar **idempotent
fire-and-forget**: `eventId === refId` (stabil kayıt uuid'si) → backend dedup BİR kez
kredi verir; başarısız gönderim ekran re-mount'unda zararsızca tekrarlanır. Hata yutulur
(kullanıcı akışı bloklanmaz). Kredi gerçekleşince `['gamification']` + `['daily-questions']`
invalidate edilir.

§7'deki aksiyonların durumu: (1) ✅ uuid, (3) ✅ daily offline-resume + 429 retry,
(4) ✅, (5) ✅ `newBadges` `{id,tier,icon}[]` (`types/gamification.ts`), (6) ✅ flame/
trophy/medal MAPPING, (7) ✅ summary + event zod şeması. Kalan: (2) CLAUDE.md Drizzle→
Prisma notu, (8) cihazda push tap-routing e2e.

### Ateşlenen olaylar ve refId kaynakları

| Olay                        | Nerede                        | refId / eventId        | Durum                                      |
| --------------------------- | ----------------------------- | ---------------------- | ------------------------------------------ |
| `training_complete`         | `result.tsx` (sınav geçince)  | `assignmentId` (route) | ✅ Canlı                                   |
| `feedback_submit`           | `feedback/[attemptId].tsx`    | `responseId` (submit)  | ✅ Canlı                                   |
| `exam_pass`                 | `result.tsx` (forward-compat) | `data.attemptId`       | ⏳ Backend results'a `attemptId` ekleyince |
| `training_complete` (SCORM) | — (mobilde assignmentId yok)  | —                      | ⏳ Backend self-award (İstek B)            |

### Tespit edilen 2 backend boşluğu (masaüstü istek listesi §A/§B)

- **İstek A:** `GET /api/exam/[id]/results` yanıtı `attempt.id` döndürmüyor → mobil
  `exam_pass` (50p) ateşleyemiyor. Tek satır ekleme (`attemptId: attempt.id`); mobil
  alanı zaten opsiyonel bekliyor → gelince otomatik etkinleşir.
- **İstek B:** SCORM ekranı `trainingId` taşıyor, `TrainingAssignment.id` taşımıyor →
  SCORM `training_complete` ekrandan ateşlenemiyor. Backend PATCH (lessonStatus
  passed/completed) zaten assignment.id'yi bulup sertifika üretiyor; o noktada
  self-award + `seedDailyReviewForTraining` önerildi.

> Önemli bağlam: Leitner havuzu `cron/daily-quiz-push` ile **batch** seed ediliyor; yani
> event olmadan da Günün Soruları içerik bulur. Event'ler **anlık seeding + puan/streak/
> rozet ekonomisinin** tek yolu — "olmazsa içerik boş" değil, "olmazsa puan/anlık-seed yok".
